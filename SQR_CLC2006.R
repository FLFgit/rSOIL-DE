...
li <- parLapply(cl,c(1:36), function(y) {
  ...
  ras <- mergeRaster(li) 
  crs(ras) <-  "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste("./SQR/",BL,"_SQR_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
  ...
  writeRaster(ras, filename=paste("./SQR/",BL,"_SQR_5K_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
