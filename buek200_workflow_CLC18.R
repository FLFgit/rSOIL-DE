...
cor <- fread("buek_cor18_gruen.csv")
...
cor[Code_18 == 231 ,Kultur_cor:="G"]
length(unique(profil[GEN_ID %in% cor$tblZuordnu]$GEN_ID)) # 2123 GL
length(unique(profil[!GEN_ID %in% cor$tblZuordnu]$GEN_ID)) # 208 kein GL, nicht weiter berücksichtigt bei der ZUordnung
...
length(unique(match$GEN_ID)) # 1125 GEN_ID mit mind. einem Profil unter G
...
length(unique(nomatch$GEN_ID)) #998 GEN_IDS ohne passendes Bodenprofil der CORINE Nutzung
write.table(unique(nomatch$GEN_ID),"nomatch_gruen.csv") #--> 22 % der Gesamtfläche GL
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
leitboden <- profil[,.SD[BOF_NR == 1], by=GEN_ID] #435
...
profil <- profil[!GEN_ID %in% leitboden$GEN_ID] # GEN_ID die keinen Leitboden GL
...
#125: alle gleichwertig c("3054","3055")]
...
# 1 von 4 BF_ID sind NA
gef1 <- profil42[BF_ID %in% gef$V1][,.SD[.N == 1], by=GEN_ID] # 1
...
cor <- fread("buek_cor18_acker.csv")
profil <- fread("tblProfile_V06.csv",na.strings = c("", "NA"))

# gibts es zu jedem CORINE Code ein passendes Bodenprofil?
cor[Code_18 %in% c(211,243),Kultur_cor:="A"][Code_18 %in% c(221,222),Kultur_cor:="S"]
length(unique(profil[GEN_ID %in% cor$tblZuordnu]$GEN_ID)) # 2022
length(unique(profil[!GEN_ID %in% cor$tblZuordnu]$GEN_ID)) # 309 kein Acker, nicht weiter berücksichigt bei der Zuordnung
...
match <- profil[GEN_ID %in% cor$tblZuordnu & KULTUR %in% cor$Kultur_cor] # Übereinstimmende GEN_ID und Kultur
length(unique(match$GEN_ID)) # 1448
...
# NEIN 
#~~~~~~~~~~~~~~~~
# laut BÜK200 keine Ackernutzung
nomatch <- profil[!GEN_ID %in% unique(match$GEN_ID)][GEN_ID %in% unique(cor$tblZuordnu)] # NICHT Übereinstimmende GEN_ID und Kultur
length(unique(nomatch$GEN_ID)) #574 GEN_IDS ohne passendes Bodenprofil der CORINE Nutzung
write.table(unique(nomatch$GEN_ID),"nomatch.csv") #--> 14 % der Gesamtfläche Acker

#was ist leitbodenprofil??
nomatch_leit <- nomatch[,.SD[BOF_NR == 1], by=GEN_ID] # 574 --> Verwendung der entsprechenden Leitbodenprofile
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# --> Verwendung der entsprechenden Leitbodenprofile
# Leitbodenprofile der no-match GEN_ID
nomatch_leit  <- nomatch_leit[,.(GEN_ID,BF_ID)]
...
leitboden <- profil[,.SD[BOF_NR == 1], by=GEN_ID] #1036
...
write.table(profil_sim,"gleicher_Anteil_A.csv",sep=";")
...
write.table(gesamt_hor_A,"Horizonte_A.csv")

profil <- fread("tblProfile_V06.csv")
gesamt_profil_A <- profil[BF_ID %in% gesamt$BF_ID]
gesamt_profil_A[,CORINE:="A"]
write.table(gesamt_profil_A,"Profile_A.csv")
