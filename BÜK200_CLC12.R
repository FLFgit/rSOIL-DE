...
buk200 <- st_read(paste(BL,"_clc12_key_BF_ID.shp", sep=""))
key <- raster(paste(BL,"_clc12_key_BF_ID.tif", sep=""))
...
hor_nr <- lapply(c(1:10), function(x){
  ...
  writeRaster(ph.raster, paste(BL,"_pH_Hor",x,".tif", sep=""),overwrite=T)
  rm(ph.sf)
  rm(ph.raster)})
...
splitRaster(hor_nr,6,6,path="Split")
...
hor_nr <- parLapply(cl,c(1:hormax[1]), function(x){
  ...
  splitRaster(skel,6,6,path= "Split")
  splitRaster(maecht,6,6,path= "Split")
  splitRaster(bokl,6,6,path= "Split")
  
  li <- parLapply(cl,c(1:36), function(y) {
    library(raster)
...
splitRaster(GM,6,6,path= "Split")

for(i in 1:length(files)){ 
splitRaster(raster(files[i]),6,6,path= "Split")} 
...
splitRaster(nFK,6,6,path= "Split")
auflage <- raster(paste(BL,"_Auflage.tif", sep=""))
splitRaster(auflage,6,6,path= "Split")
...
recl[,1] <- c(1:11)
recl[,2] <- c(1,1,1,2,2,2,3,3,6,6,5)
slope_rc <- reclassify(slope_r,recl,rigth=NA)

splitRaster(slope_rc,6,6,path= "Split")
...
splitRaster(neig,6,6,path= "Split")
nfk <- raster(paste(BL,"_ab_nfk_O.tif",sep=""))
splitRaster(nfk,6,6,path= "Split")
hor <- raster(paste(BL,"_ab_Hor.tif",sep=""))
splitRaster(hor,6,6,path= "Split")

#Bewertung von Versiegelungsgrad/Bodenbedeckung

nutz<-raster(paste(BL,"_Nutzung_2012.tif",sep=""))

# Gruppierung der Bodenbedeckung nach Tab 26 A = 2, GL = 3
recl <- matrix(data=c(1,2,2,3),2,2) 
nutz_rc <- reclassify(nutz,recl,rigth=NA)
writeRaster(nutz_rc, filename=paste(BL,"_Nutzung2012_abfl.tif",sep=""), format="GTiff", overwrite=TRUE)

nutz_rc <- raster(paste(BL,"_Nutzung2012_abfl.tif",sep=""))
splitRaster(nutz_rc,6,6,path= "Split")
...
li <- parLapply(cl, c(1:36), function(y) {
  ...
  nutz_rc_t <- raster(paste("./Split/",BL,"_Nutzung2012_abfl_tile",y,".gri", sep=""))
...
writeRaster(abfl, filename=paste(BL,"_abfl_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
...
hor_nr <- parLapply(cl,c(1:hormax[1]), function(x){
  ...
  maecht <- raster(paste(BL,"_Mächtigkeit_Hor",x,".tif", sep=""))
  korrbew <- raster(paste(BL,"_korrBEW_Hor",x,".tif", sep=""))
  ba <- raster(paste(BL,"_BA_Hor",x,".tif", sep=""))
  splitRaster(maecht,6,6,path= "Split")
  splitRaster(korrbew,6,6,path= "Split")
  splitRaster(ba,6,6,path= "Split")
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    ...
    kbew_t <- raster(paste("./Split/",BL,"_korrBEW_Hor",x,"_tile",y,".gri", sep=""))
    ...
    tile <- calc(st, fun=function(z) {
      ifelse(z[1] %in% c(8,36,26,6:7,15,22,23,1:3), 1*z[2],
      ...
      ifelse(z[1] ==13 & z[3] %in% c(19,47) , 6*z[2],         # Ut4|Lu
          ifelse(z[1] %in% c(33,40,41,47:49,29:36), 1*z[2],
          ifelse(z[1] %in% c(36,62,50:52), 2*z[2],
...
splitRaster(wölb,6,6,path= "Split")
eaqua_r <- raster(paste(BL,"_korrBEW_Eaqua.tif",sep=""))
splitRaster(eaqua_r,6,6,path= "Split")
...
key_r<-raster(paste(BL,"_clc12_key_BF_ID.tif",sep=""))
slope_r<-raster(paste(BL,"_Slope_10.tif",sep=""))
...
splitRaster(slope,6,6,path= "Split")
...
splitRaster(p_so,6,6,path= "Split")
...
nutz<-raster(paste(BL,"_Nutzung_2012.tif",sep=""))
splitRaster(nutz, 6,6,path= "Split")

# Nutzungsabhängige Korrektur der Bodenabtragswerte
abtrag <- raster(paste(BL,"_abtrag.tif",sep=""))
splitRaster(abtrag,6,6,path= "Split")
...
li <- parLapply(cl,c(1:36), function(y) {
  ...
  nutz_t <- raster(paste("./Split/",BL,"_Nutzung_2012_tile",y,".gri", sep=""))
...
crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"  
writeRaster(ras, filename=paste(BL,"_erwa_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
...
hormax <- max(values(nr_hor),na.rm=T) # benötigte Anzahl an Horizonten für Berechnung
...
hor_nr <- parLapply(cl,horm, function(x){
  ...
  #splitRaster(nr_hor,6,6,path= "Split")
  splitRaster(gw,6,6,path= "Split")
...
hor_nr <- parLapply(cl,c(1:hormax[1]), function(x){
  ...
  getDoParVersion()
    ...
    #splitRaster(ba,6,6,path= "Split")
    splitRaster(humus,6,6,path= "Split")
    #splitRaster(ld,6,6,path= "Split")
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    ...
    tile <- calc(st, function(z) {
      ...
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(0:3) & z[3] %in% c(1,2), 6*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(0:3) & z[3] == 3 , 5*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(0:3) & z[3] == 4, 4*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(0:3) & z[3] == 5 , 3*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(0:3) & z[3] %in% c(6,7), 1*z[4],
      ...
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(4:7) & z[3] %in% c(1,2), 6*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(4:7) & z[3] %in% c(3,4) , 5*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(4:7) & z[3] == 4 , 4*z[4],
      ifelse(z[1] %in% c(25,36:34) & z[2] %in% c(4:7) & z[3] %in% c(6,7), 1*z[4],
...
ras <- reclassify(ras,recl,right=NA)
    ...
    crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
    writeRaster(ras,filename=paste(BL,"_e_wind_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
...
bük<- fread("../../GW_Abstand_BÜK1000.csv", sep=";", dec=",", header = T)
bük <- bük[,max(GW_Abstand),by=BF_ID] # maximale Tiefe je BF_ID
...
hor_nr <- parLapply(cl, horm, function(x){
  ...
  #li <- lapply(list.files(path=".", pattern="li_2_"), raster) 
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
...
#     
#   })
#   #li <- lapply(list.files(path=".", pattern="li_1_"), raster) 
#   ras <- mergeRaster(li) 
#   crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
#   writeRaster(ras, filename=paste(BL,"_mF_hor_",x,".tif",sep=""), format="GTiff", overwrite=TRUE)
#   
...
splitRaster(mgw,6,6,path= "./Split")

mittel <- raster(paste(BL,"_mF_mittel.tif",sep=""))

splitRaster(mittel,6,6,path= "./Split")
...
splitRaster(klim_wb,6,6,path= "./Split")
splitRaster(zus_f,6,6,path= "./Split")
...
free.kernel (cl)  
  ...
  splitRaster(f_wb,6,6,path= "./Split")
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_MF_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
...
li <- parLapply(cl,c(1:36), function(y) {
   ...
   # Rundung bei .5 wird aufgerundet
    ...
    #li <- lapply(list.files(path=".", pattern=paste("li_",sep="")), raster)
    ras <- mergeRaster(li) 
    crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
    ...
    mgw_st <- stack(mittel_t,mgw_t)
		...
		tile <- calc(mgw_st, fun=function(x) { ifelse(x[2]<80,x[1]-2,
                                               ifelse (x[2]>80 & x[2]<200,x[1]-1,
                                                       ifelse (x[2]>1000 & x[2]<3000,x[1]+1,
                                                               ifelse (x[2]>=3000,x[1]+2,x[1]))))}) 
    tile2 <- stack(tile,mittel_t)
		tile <- calc(tile2, fun=function(x) {ifelse(is.na(x[1]),x[2], x[1])})
...
# keine Bewertung >5
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    ...
    zus_f <- raster(paste("./Split/",BL,"_pcF_zus_f_tile",y,".gri", sep=""))
		tile <- calc(zus_f, fun=function(x) { ifelse (x<=1,1,ifelse(x>5,5,x))})
		crs(tile) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
		writeRaster(tile, filename=paste("li_",y,".tif",sep=""), format="GTiff", overwrite=TRUE)
		 })
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_pcF_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
...
# Einladen Nutzung
nutz<-raster(paste(BL,"_Nutzung_2012.tif",sep="")) # Acker = 1, GL = 2
...
hor_nr <- parLapply(cl,c(1:hormax[1]), function(x){
  ...
  bok <- raster(paste(BL,"_Boklasse_Hor",x,".tif", sep=""))
  #maecht <- raster(paste(BL,"_Mächtigkeit_Hor",x,".tif", sep=""))
  splitRaster(bok,6,6,path= "Split")
  #splitRaster(maecht,6,6,path= "Split")
...
hor_nr <- parLapply(cl,c(1:hormax[1]), function(x){
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    ...
    tile <- calc(ba_tk, fun=function(z) {ifelse(z %in% c(6:12,20:22), 1,  #sehr gering ss
                                      ifelse(z %in% c(25,26,28,30,32), 2,  # us, ls
                                      ifelse(z %in% c(44,33,34,48,27,15,31), 3,  # us, ls, su, sl
                                      ifelse(z %in% c(13,14,43,45:47,19), 4,    # lu, sl 
                                      ifelse(z %in% c(40:42,16:18,35:39), 5, NA)))))}) # tl, tu, lt, ut
    ...
    tile3 <- calc(st, fun=function(z){ifelse(z[2]==2 & z[1]<5, (z[1]+1)*z[3], z[1]*z[3])}) # GL-Bewertung jeweils 1 höher als Acker
...
free.kernel (cl)  
  ...
  splitRaster(stau,6,6,path= "Split")
...
free.kernel (cl)
  ...
  NAvalue(Temp) <- -999
  splitRaster(Temp,6,6,path= "Split")
...
free.kernel (cl)
  ...
  ewa <- raster(paste(BL,"_erwa_2012.tif",sep=""))
  splitRaster(ewa,6,6,path= "Split")
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    library(raster)
    BL <- "BE" 
    ewa_t <- raster(paste("./Split/",BL,"_erwa_2012_tile",y,".gri", sep=""))
...
free.kernel (cl)
  ...
  parLapply(cl,1:length(files), function(y) {
    library(raster)
    library(SpaDES.tools)
    BL <- "BE"
    files <- list.files(path=".", pattern=paste(BL,"_be_",sep=""), full.names = T)
    splitRaster(raster(files[y]),6,6,path= "Split")}) 
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_bio_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
