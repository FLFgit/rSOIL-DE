...
length(unique(match$GEN_ID)) # 1124 GEN_ID mit mind. einem Profil unter GL
...
length(unique(nomatch$GEN_ID)) #999 GEN_IDS ohne passendes Bodenprofil der CORINE Nutzung
write.table(unique(nomatch$GEN_ID),"nomatch_gruen.csv",row.names=F,sep=";") 

#was ist leitbodenprofil??
nomatch_leit <- nomatch[,.SD[BOF_NR == 1], by=GEN_ID] # 999 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
profil22 <- profil22[!GEN_ID %in% gef$GEN_ID] # 27
...
cor <- fread("bük200_clc12_A.csv")
profil <- fread("../tblProfile_V06.csv",na.strings = c("", "NA"))

# gibts es zu jedem CORINE Code ein passendes Bodenprofil?
cor[Code_12 %in% c(211,243,242),Kultur_cor:="A"][Code_12 %in% c(221,222),Kultur_cor:="S"]
...
match <- profil[GEN_ID %in% cor$tblZuordnu & KULTUR %in% cor$Kultur_cor] # Übereinstimmende GEN_ID und Kultur
length(unique(match$GEN_ID)) # 1449
...
length(unique(nomatch$GEN_ID)) #573 GEN_IDS ohne passendes Bodenprofil der CORINE Nutzung
write.table(unique(nomatch$GEN_ID),"nomatch.csv",row.names=F,sep=";") 

#was ist leitbodenprofil??
nomatch_leit <- nomatch[,.SD[BOF_NR == 1], by=GEN_ID] # 573 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# --> Verwendung der entsprechenden Leitbodenprofile
# Leitbodenprofile der no-match GEN_ID
nomatch_leit  <- nomatch_leit[,.(GEN_ID,BF_ID)]
...
leitboden <- profil[,.SD[BOF_NR == 1], by=GEN_ID] #1038
...
# GEN_ID mit 2 Bodenprofilen (geteilt durch 2)
...
profil2 <- profil2[,.SD[sample(.N, 1)], by = GEN_ID]
# Summe ausgewählter BF_IDs
length(unique(profil2$GEN_ID)) #90
...
write.table(gesamt_profil_A,"Profile_A.csv",row.names=F,sep=";")
