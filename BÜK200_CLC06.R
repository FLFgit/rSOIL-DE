...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
BL <- "BY" # alle im Skript ersetzen Strg+F
...
buk200 <- st_read(paste(BL,"_clc06_key_BF_ID.shp", sep=""))
key <- raster(paste(BL,"_clc06_key_BF_ID.tif", sep=""))
...
### gewichtetes Mittel über Horizontmächtigkeit
...
nutz<-raster(paste(BL,"_Nutzung2006.tif",sep=""))

# Gruppierung der Bodenbedeckung nach Tab.26 A = 2, GL = 3
recl <- matrix(data=c(1,2,2,3),2,2) 
nutz_rc <- reclassify(nutz,recl,rigth=NA)
writeRaster(nutz_rc, filename=paste(BL,"_Nutzung2006_abfl.tif",sep=""), format="GTiff", overwrite=TRUE)

nutz_rc <- raster(paste(BL,"_Nutzung2006_abfl.tif",sep=""))
...
li <- parLapply(cl, c(1:36), function(y) {
  ...
  nutz_rc_t <- raster(paste("./Split/",BL,"_Nutzung2006_abfl_tile",y,".gri", sep=""))

st <- stack(hor_t,neig_t,nfk_t,nutz_rc_t)
abfluss <- calc(st, fun=function(x) sum(x))

# Klassifizierung nach BA-LVL
...
# Umkehren der Bewertung damit einheitlich 1=sehr gering und 5=sehr hoch
recl <- matrix(data=NA,5,2)
recl[,1] <- c(1:5)
recl[,2] <- c(5,4,3,2,1)
abfl <- reclassify(ras, recl, right=NA)

writeRaster(abfl, filename=paste(BL,"_abfl_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
...
# !!! Moore werden nicht berücksichtigt in BA LVL !!! --> haben aber höchsten Erosionswiderstand gegen Wasser --> Klasse 2
...
# Bewertung des Bodenartbedingten Erosionswiderstandes (BEW)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

tab1<- fread("../Tab1.csv", sep=";", dec=",", header = T)

# Zuweisung BEW nur für Oberböden (mind. 0-10 cm)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Code aus Humus, Skelett und BEW für korr. BEW (vier od- fünfstellig (Humus 1-3, Skelett 10-100, BEW=1-8), dann zweistellig (nach as. numeric)
...
li <- parLapply(cl,c(1:36), function(y) {
  ...
  nutz_t <- raster(paste("./Split/",BL,"_Nutzung2006_tile",y,".gri", sep=""))
...
crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"  
writeRaster(ras, filename=paste(BL,"_erwa_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
...
ras <- reclassify(ras,recl,right=NA)
    ...
    crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
    writeRaster(ras,filename=paste(BL,"_e_wind_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
...
free.kernel (cl)  
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_MF_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
...
# keine Bewertung >5
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_pcF_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
...
# Einladen Nutzung
nutz<-raster(paste(BL,"_Nutzung2006.tif",sep="")) # Acker = 1, GL = 2
...
free.kernel (cl)
  ...
  ewa <- raster(paste(BL,"_erwa_2006.tif",sep=""))
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    library(raster)
    BL <- "BY" 
    ewa_t <- raster(paste("./Split/",BL,"_erwa_2006_tile",y,".gri", sep=""))
...
free.kernel (cl)
  ...
  ras <- mergeRaster(li) 
  crs(ras) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste(BL,"_bio_2006.tif",sep=""), format="GTiff", overwrite=TRUE)
