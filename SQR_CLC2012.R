...
splitRaster(slope_r, 6,6, path="Split")
...
splitRaster(GM_A,6,6,path= "Split")
splitRaster(GM_GL,6,6,path= "Split")
...
splitRaster(nr_A,6,6,path= "Split")
splitRaster(nr_GL,6,6,path= "Split")
...
hor_nr <- parLapply(cl,c(1:hormax[1] ), function(x){
  ...
  li <- parLapply(cl,c(1:36), function(y) {
    ...
    crs(tile) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
    writeRaster(tile, filename=paste("ri_",x,"_",y,".tif",sep=""), format="GTiff", overwrite=TRUE)
    })
...
splitRaster(depth_A,6,6,path= "Split")
splitRaster(depth_GL,6,6,path= "Split")
...
splitRaster(root,6,6,path= "./Split")
...
ri <- parLapply(cl,c(1:36), function(y) {
    library(raster)
    BL <- "BE"
    ...
    crs(tile) <- "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
    writeRaster(tile, filename=paste("ri_",y,".tif",sep=""), format="GTiff", overwrite=TRUE)
...
splitRaster(klim,6,6,path= "./Split")
...
splitRaster(risk,6,6,path= "Split")
...
splitRaster(rock,6,6,path= "./Split")
...
splitRaster(ske,6,6,path= "./Split")
...
splitRaster(hazard,6,6,path= "Split")
basic <- raster("./SQR/SQR_basic.tif")
splitRaster(basic,6,6,path= "Split")  
...
li <- parLapply(cl,c(1:36), function(y) {
  ...
  ras <- mergeRaster(li) 
  crs(ras) <-  "+proj=utm +zone=32 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs"
  writeRaster(ras, filename=paste("./SQR/",BL,"_SQR_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
  ...
  writeRaster(ras, filename=paste("./SQR/",BL,"_SQR_5K_2012.tif",sep=""), format="GTiff", overwrite=TRUE)
